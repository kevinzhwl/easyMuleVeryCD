/* 
 * $Id: DownloadQueue.cpp 19920 2010-06-22 06:50:01Z dgkang $
 * 
...
BOOL CDownloadQueue::ImportPartFile(CString strPartFilePath)
{
	BOOL bMetFile = FALSE,bPartFile = FALSE;
	CString strTemp;

	if (strPartFilePath.GetLength() >  5)
	{
		strTemp = strPartFilePath.Right(5);
		if( !strTemp.CompareNoCase( _T(".part")) )
			bPartFile = TRUE;
	}

	if (!bPartFile && strPartFilePath.GetLength() > 9)
	{
		strTemp = strPartFilePath.Right(9);
		if( !strTemp.CompareNoCase(_T(".part.met")) )
			bMetFile = TRUE;
	}

	if (!bMetFile && !bPartFile)
	{
		return FALSE;
	}

	CString strMetFilePath  = _T("");
	CString strDirectory	= strPartFilePath.Left(strPartFilePath.ReverseFind(_T('\\')));
	CString strMetFileName	= _T("");

	if (bMetFile)
	{
		strMetFilePath = strPartFilePath;
	}
	else
	{
		strTemp = strPartFilePath.Mid(strPartFilePath.ReverseFind(_T('\\')) + 1);
		strTemp = strTemp.Left(strTemp.GetLength() - 5);
		strMetFilePath  = strDirectory +  _T("\\") + strTemp + _T(".part.met");
	}
	strMetFileName = strMetFilePath.Mid(strMetFilePath.ReverseFind(_T('\\')) + 1);

	CPartFile* toadd = new CPartFile();
	if(GetFileByMetFileName(strMetFilePath) != NULL)
	{
		delete toadd;
		return FALSE;
	}
	else if (toadd->LoadPartFile(strDirectory,strMetFileName,false,false,false))
	{
		bool bCanAdd = true;
		if(toadd->GetStatus(true) == PS_READY)
		{
			bCanAdd = CGlobalVariable::sharedfiles->SafeAddKFile(toadd);			
		}
		...
		if (toadd->HasNullHash())
		{
			CString url = toadd->GetPartFileURL();
			bCanAdd = CGlobalVariable::filemgr.GetUrlTaskState(url) == FILESTATE_NOT_EXIST;
		}
		...
		if(bCanAdd)
		{
			...
			if( toadd->m_bNeedReHash )
			{
				toadd->ReHashPartFile();
				toadd->m_bNeedReHash = FALSE;
			}
		}
		else
		{
...
void CDownloadQueue::ScanPartFile(CString strDirectory)
{
	...
	while (!end)
	{
		...
		else if (toadd->LoadPartFile(strDirectory, ff.GetFileName(),false,false,false))
		{
			bool bCanAdd = true;
...
bool CDownloadQueue::IsFileExisting(const uchar* fileid, bool bLogWarnings,bool *pbIsPartFile) const
{	
	if (isnulmd4(fileid)) // VC-SearchDream[2007-03-21]: For HTTP and FTP Direct DownLoad
	{
		return false;
	}
	...
	if (file)
	{
		...
		if( pbIsPartFile!=NULL )
			*pbIsPartFile = file->IsPartFile();
		return true;
	}
...
bool CDownloadQueue::CheckAndAddSource(CPartFile* sender,CUpDownClient* source,bool bcheckdead,bool bInsertHead/*=false*/)
{
	...
	if (source->HasValidHash())
	{
		...
		if (bcheckdead && CGlobalVariable::clientlist->m_globDeadSourceList.IsDeadSource(source) || sender->m_DeadSourceList.IsDeadSource(source))
		{
...
void CDownloadQueue::ResetLocalServerRequests()
{
	...
	{ 
		...
		//ADDED BY VC-ZHUHUI ON 2010-06-04 <BEGIN>:FTP下载没有HASH
		if( (uState == PS_READY || uState == PS_EMPTY) && (!pFile->HasNullHash() || pFile->GetINetDownLoadURL().CompareNoCase(_T("ftp://")))/*pFile->GetPartFileSizeStatus() == FS_NOSIZE )*/)
		{
			pFile->ResumeFile();
		}
        //ADDED BY VC-ZHUHUI ON 2010-06-04 <END>:
	...
	while (NULL != pos)
	{
		...
		if (NULL != pPartFile)
		{
			if ( !pPartFile->IsStopped()
				&& !pPartFile->IsPaused() && (pPartFile->getPartfileStatusRang()==2 || pPartFile->getPartfileStatusRang()==3) )
			{
				bRunning = TRUE;
				break;
			}
