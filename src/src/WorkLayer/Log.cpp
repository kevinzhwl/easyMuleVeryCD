/* 
 * $Id: Log.cpp 20689 2010-10-20 04:12:33Z dgkang $
 * 
...
void AddLogTextV(UINT uFlags, EDebugLogPriority dlpPriority, LPCTSTR pszLine, va_list argptr)
{
	...
	CString szLine = pszLine;
	try
	{
		TCHAR szLogLine[1000];
#ifdef	__STDC_SECURE_LIB__
		if (_vsntprintf_s(szLogLine, ARRSIZE(szLogLine), _TRUNCATE, szLine, argptr) == -1)
			szLogLine[ARRSIZE(szLogLine) - 1] = _T('\0');
#else
		if (_vsntprintf(szLogLine,ARRSIZE(szLogLine), szLine, argptr) == -1)
			szLogLine[ARRSIZE(szLogLine) - 1] = _T('\0');
#endif
