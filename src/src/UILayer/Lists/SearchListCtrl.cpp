/* 
 * $Id: SearchListCtrl.cpp 20835 2010-11-18 10:36:09Z dgkang $
 * 
...
void CSearchListCtrl::AddResult(const CSearchFile* toshow)
{
	...
	m_stringMatcher.AddItem((void*)toshow, toshow->GetFileName());
}
...
void CSearchListCtrl::ShowFileSearchResults( CFileSearch* pFileSearch)
{

}

void CSearchListCtrl::ShowResults(uint32 nResultsID,CFileSearch* pFileSearch)
{
	...
	DeleteAllItems();
	m_stringMatcher.RemoveAllItems(); // 切换搜索标签页时同时清空字符串匹配内容 added by ck
...
//		thePrefs.SetColumnSortAscending(CPreferences::tableSearch, pNewState->m_bSortAscending);
		...
		if(NULL==pFileSearch)
			...
			ShowFileSearchResults( pFileSearch );
...
void CSearchListCtrl::FilterBySearchKey(CString* pSearchKey)
{
	if (!IsWindowVisible())
		return;
	...
	{
		CSearchFile* toshow = (CSearchFile*)*ix;
