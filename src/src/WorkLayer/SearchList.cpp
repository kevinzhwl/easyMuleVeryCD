/* 
 * $Id: SearchList.cpp 18513 2010-03-24 09:41:45Z huby $
 * 
...
void CSearchList::AddResults( SearchListsStruct * pSearchListsStruct )
{
	m_listFileLists.AddTail( pSearchListsStruct );
}
...
void CSearchList::ShowResults(uint32 nSearchID)
{
	...
	int n = pSearchFileList->GetCount();
...
void CSearchList::NewSearch(CSearchListCtrl* /*pWnd*/, CStringA strResultFileType, uint32 nSearchID, ESearchType eSearchType, CString strSearchExpression, bool bMobilMuleSearch)
{
	...
	if (eSearchType == SearchTypeEd2kServer || eSearchType == SearchTypeEd2kGlobal || eSearchType == SearchTypeEasyMuleFile){
...
bool CSearchList::AddToList(CSearchFile* toadd, bool bClientResponse, uint32 dwFromUDPServerIP,bool bWM_AddResult,bool bVerifyWordFilter,bool bDoSpamRating)
{
	...
	if(bVerifyWordFilter && !WordFilter.VerifyString(toadd->GetFileName()))
	{
		delete toadd;
		return false;
	}
	...
	if (!m_MobilMuleSearch && bWM_AddResult)
	{
