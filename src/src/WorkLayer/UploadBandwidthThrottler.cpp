/* 
 * $Id: UploadBandwidthThrottler.cpp 19524 2010-05-20 10:09:21Z dgkang $
 * 
...
uint32 UploadBandwidthThrottler::GetSlotLimit(uint32 currentUpSpeed) {
	...
	if( upPerClient > max(DynamicPref.m_iUpPerClient,3072) )
		upPerClient = max(DynamicPref.m_iUpPerClient,3072);
...
#else
		// edit by jimmyc [2010-4-8]
		if ( CGlobalVariable::s_IntelligentUploadManager != NULL && !CGlobalVariable::s_IntelligentUploadManager->IsLimitUploadSpeed() )
		{
			allowedDataRate = 0xFFFFFFFF;			
		}
		else
		{
