/* 
 * $Id: MuleListCtrl.cpp 20251 2010-07-22 06:49:14Z dgkang $
 * 
...
// Modified by VC-yavey on 2010-06-22 <begin>
// if we add a new column in list, we should not use last config
void CMuleListCtrl::LoadSettings(BOOL bCheckColNum)
{
	ASSERT(!m_Name.IsEmpty());
	if (m_Name.IsEmpty())
		return;
	...
	{
		int iConfigNum = 1;
		CString strWidths = ini.GetString(m_Name + _T("ColumnWidths"), _T(""));
		int iPos = strWidths.Find(',');
		while ( iPos >= 0 )
		{
			iConfigNum++;
			iPos = strWidths.Find(',', iPos + 1);
		}
		
		if ( iConfigNum != pHeaderCtrl->GetItemCount() )
		{
			return;
		}
	}
...
// Modified by VC-yavey on 2010-06-22 <end>
...
void CMuleListCtrl::SetRssArrow(int iColumn, ArrowType atType, CString strPoster)
{
	HDITEM headerItem;
	headerItem.mask = HDI_FORMAT;
	CHeaderCtrl* pHeaderCtrl = GetHeaderCtrl();

	if(iColumn != 0&&iColumn != m_iCurrentSortItem) {
	...
	if(iColumn >= 0 && pHeaderCtrl->GetItem(iColumn, &headerItem)) 
	{
		...
		if (hInstRes != NULL){
			...
			if (hbmSortStates != NULL){
				...
				if (imlSortStates.Create(14, 14, theApp.m_iDfltImageListColorFlags | ILC_MASK, 1, 0)){
					...
					//加载海报展开图片 by huhg 100707
					HINSTANCE hInstResColl = AfxFindResourceHandle(MAKEINTRESOURCE(ArrowType::arrowCollapse), RT_BITMAP);
					HINSTANCE hInstResExpand = AfxFindResourceHandle(MAKEINTRESOURCE(ArrowType::arrowExpand), RT_BITMAP);
					if (hInstResColl&&hInstResExpand)
					{
						HBITMAP hbmSortState_Coll = (HBITMAP)::LoadImage(hInstResColl, MAKEINTRESOURCE(ArrowType::arrowCollapse), IMAGE_BITMAP, 0, 0, LR_CREATEDIBSECTION | LR_LOADMAP3DCOLORS);
						HBITMAP hbmSortStates_Expand = (HBITMAP)::LoadImage(hInstResExpand, MAKEINTRESOURCE(ArrowType::arrowExpand), IMAGE_BITMAP, 0, 0, LR_CREATEDIBSECTION | LR_LOADMAP3DCOLORS);

						if (hInstResColl&&hInstResExpand)
						{
							CBitmap bmSortState_Coll;
							bmSortState_Coll.Attach(hbmSortState_Coll);
							CBitmap bmSortState_Expand;
							bmSortState_Expand.Attach(hbmSortStates_Expand);

							VERIFY( imlSortStates.Add(&bmSortState_Coll, RGB(255, 0, 255)) != -1 );
							VERIFY( imlSortStates.Add(&bmSortState_Expand, RGB(255, 0, 255)) != -1 );
						}
					}
					//加载海报展开图片 by huhg 100707
		...
		if (iColumn == 0)//海报列
		{
			...
			headerItem.fmt |= HDF_IMAGE| HDF_BITMAP_ON_RIGHT;
			headerItem.pszText = const_cast<LPTSTR>(static_cast<LPCTSTR>(strPoster));
			if ( atType == ArrowType::arrowCollapse )
				headerItem.iImage = 1;
			else
				headerItem.iImage = 2;
		}
