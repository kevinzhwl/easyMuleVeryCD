/* 
 * $Id: DownloadListCtrl.cpp 20835 2010-11-18 10:36:09Z dgkang $
 * 
...
void CDownloadListCtrl::AddFile(CPartFile* toadd)
{
	...
	if (toadd->CheckShowItemInGivenCat(curTab) && thePrefs.m_LastUpdateHash.Compare(md4str(toadd->GetFileHash())) != 0)
	{
		CString fileName = toadd->GetFileName();

		// 只显示未过滤或符合显示条件(智能过滤中)的新增任务
		if (m_stringMatcher.IsOriginal() || m_stringMatcher.GetOneItemMatchResult(fileName))
			InsertItem(LVIF_PARAM|LVIF_TEXT,itemnr,LPSTR_TEXTCALLBACK,0,0,0,(LPARAM)newitem);

		m_stringMatcher.AddItem((void*)toadd, fileName);
	}
	...
	__try
	{
		...
		for(ListItems::iterator it = m_ListItems.begin(); it != m_ListItems.end(); ){
			...
			if(delItem->owner == toremove || delItem->value == (void*)toremove){
				// Remove it from the m_ListItems
				it = m_ListItems.erase(it);
				m_stringMatcher.RemoveItem((void*)toremove);
...
void CDownloadListCtrl::DrawFileItem(CDC *dc, int nColumn, LPCRECT lpRect, CtrlItem_Struct *lpCtrlItem)
{
	if(lpRect->left < lpRect->right)
	{
	   ...
	   case 1:		
		...
		case 2:		// size			
			if ( lpPartFile->FileSizeIsKnown() && lpPartFile->GetFileSize()>(uint64)0 )
			{
		...
		case 6:		// progress
			{
				...
				if (thePrefs.GetUseDwlPercentage()) {
					...
					if ( lpPartFile->GetPartFileSizeStatus() == FS_NOSIZE && percent < 100.0f)
					{
						buffer.Format(_T("%.1f%%"), 0.0f);
					}
					else
					{
...
void CDownloadListCtrl::UpdateCurrentCategoryView(CPartFile* thisfile) {
	CString filename = thisfile->GetFileName();

	if (filename != m_stringMatcher.GetItemName((void*)thisfile))
	{
		// metlink获取到新文件名时修改内部匹配数据
		m_stringMatcher.RenameItem((void*)thisfile, filename);
	}
	else if (!m_stringMatcher.IsOriginal())
	{
		// 处于过滤状态且文件名没有变化,则不响应未显示任务的刷新
		return;
	}
	...
	if (it != m_ListItems.end()) {
		...
		if (cur_item->type == FILE_TYPE){
			...
			if (!file->CheckShowItemInGivenCat(curTab) || !m_stringMatcher.GetOneItemMatchResult(filename))
				HideFile(file);
			else
				ShowFile(file);
		}
...
void CDownloadListCtrl::ShowFile(CPartFile* toshow){
	...
	if(it != rangeIt.second){
		...
		//added by lost 隐藏升级任务
		if (result == -1 && thePrefs.m_LastUpdateHash.Compare(md4str(toshow->GetFileHash())) != 0)
...
LRESULT CDownloadListCtrl::WindowProc(UINT message, WPARAM wParam, LPARAM lParam)
{
...
void CDownloadListCtrl::FilterItemsBySearchKey(CString* pSearchKey)
{
	if (!IsWindowVisible())
		return;
	...
	const StringMatcher::ListItemsNeedShow& itemsNeedShow = m_stringMatcher.GetMatchResult(pSearchKey);

	SetRedraw(FALSE);

	DeleteAllItems();

	for (StringMatcher::ListItemsNeedShow::const_iterator ix = itemsNeedShow.begin(); ix != itemsNeedShow.end(); ++ix)
	{
		CPartFile* listKey = (CPartFile*)*ix;
		CtrlItem_Struct* item = (CtrlItem_Struct*)m_ListItems.find(listKey)->second;
		int itemnr = GetItemCount();
		InsertItem(LVIF_PARAM|LVIF_TEXT,itemnr,LPSTR_TEXTCALLBACK,0,0,0,(LPARAM)item);
	}
