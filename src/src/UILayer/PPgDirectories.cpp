/* 
 * $Id: PPgDirectories.cpp 12904 2009-05-15 06:21:43Z dgkang $
 * 
...
BEGIN_MESSAGE_MAP(CPPgDirectories, CPropertyPage)
	...
	ON_BN_CLICKED(IDC_FLUSHSHAREDIR, OnBnClickedFlushsharedir)
	ON_EN_CHANGE(IDC_INTERVAL_EDIT, OnEnChangeIntervalEdit)
END_MESSAGE_MAP()
...
void CPPgDirectories::LoadSettings(void)
{
	...
	CheckDlgButton(IDC_FLUSHSHAREDIR,thePrefs.m_bAutoFlushShareDir);
	SetDlgItemInt(IDC_INTERVAL_EDIT,thePrefs.m_nAutoFlushInterval);
	GetDlgItem(IDC_INTERVAL_EDIT)->EnableWindow(IsDlgButtonChecked(IDC_FLUSHSHAREDIR));
}
...
*/
	...
	thePrefs.m_bAutoFlushShareDir = IsDlgButtonChecked(IDC_FLUSHSHAREDIR);

	UINT n  = GetDlgItemInt(IDC_INTERVAL_EDIT);
	if (n <=0 ) n = 1;
	if (n >60 ) n = 60;

	thePrefs.m_nAutoFlushInterval = n;
	SetDlgItemInt(IDC_INTERVAL_EDIT,n);
...
void CPPgDirectories::Localize(void)
{
	if(m_hWnd)
	{
		...
		GetDlgItem(IDC_FLUSHSHAREDIR)->SetWindowText(GetResString(IDS_FLUSHSHAREDIR));
		GetDlgItem(IDC_OPTION_FRM)->SetWindowText(GetResString(IDS_FLUSHDIR_OPTION));
		GetDlgItem(IDC_INTERVAL_LAB)->SetWindowText(GetResString(IDS_FLUSHDIR_INTERVAL));
...
void CPPgDirectories::OnEnChangeTempfiles()
{
...
void CPPgDirectories::OnBnClickedFlushsharedir()
{
	// TODO: Add your control notification handler code here
	GetDlgItem(IDC_INTERVAL_EDIT)->EnableWindow(IsDlgButtonChecked(IDC_FLUSHSHAREDIR));
	SetModified();
...
void CPPgDirectories::OnEnChangeIntervalEdit()
{
