/* 
 * $Id: UploadQueue.cpp 19524 2010-05-20 10:09:21Z dgkang $
 * 
...
{
	//modify by auona on 2010/05/10 <begin>
	POSITION toadd = 0;
	//POSITION toaddlow = 0;
	uint32	bestscore = 0;
    ...
    if (newclient && newclient->HasLowID())
    {
	...
	*/
	//modify by auona on 2010/05/10 <end>
}
...
bool CUploadQueue::AddUpNextClient(LPCTSTR pszReason, CUpDownClient* directadd){
	...
	{
		if(newclient->socket)
			AddDebugLogLine(false, _T("[+%s+UseNat(%d)]Adding client to upload list: %s Client: %s"),newclient->HasLowID()?_T("LowId"):_T("HighId"),newclient->socket->m_bUseNat,pszReason, newclient->DbgGetClientInfo());
		else
			AddDebugLogLine(false, _T("[+%s+]Adding client to upload list: %s Client: %s"),newclient->HasLowID()?_T("LowId"):_T("HighId"),pszReason, newclient->DbgGetClientInfo());
	}
...
void CUploadQueue::AddClientToQueue(CUpDownClient* client, bool bIgnoreTimelimit)
{
   ...
   // cap the list
	...
	if( DynamicPref.m_iWaitQueueThreshold>=800 && DynamicPref.m_iWaitQueueThreshold<=4000 )
	{
		softQueueLimit = DynamicPref.m_iWaitQueueThreshold;
		hardQueueLimit = DynamicPref.m_iWaitQueueThreshold + max(thePrefs.GetQueueSize()/4, 200);
	}
	else
	{
		softQueueLimit = thePrefs.GetQueueSize();
		hardQueueLimit = thePrefs.GetQueueSize() + max(thePrefs.GetQueueSize()/4, 200);
	}
	...
	try
	{
		...
		if (counter >= 10){
           ...
           CGlobalVariable::uploadqueue->UpdateDatarates();
			...
			DynamicPref.GetDynamicPref();

//#if 0		
			if(thePrefs.m_bAutoFlushShareDir && thePrefs.m_nAutoFlushInterval > 0)
			{
				static int auto_flush_sharedir_time = 0;
				auto_flush_sharedir_time++;
				if (auto_flush_sharedir_time > thePrefs.m_nAutoFlushInterval * 60)
				{
					auto_flush_sharedir_time = 0;

					if(theApp.emuledlg->sharedfileswnd && theApp.emuledlg->sharedfileswnd->sharedfilesctrl.GetItemCount() > 0)
						CGlobalVariable::sharedfiles->AutoReload(TRUE);
					else
						CGlobalVariable::sharedfiles->AutoReload(FALSE);
				}
			}
			//////////////////////////////////////////////////////////////////////////
