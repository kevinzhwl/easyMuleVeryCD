/*
 * $Id: DlgMainTabResource.cpp 20689 2010-10-20 04:12:33Z dgkang $
 * 
...
void CDlgMainTabResource::OnNMActiveTabChanged(NMHDR* pNMHDR, LRESULT *pResult)
{
	...
	if (TCD_WEB_BROWSER == CustomData)
	{
		...
		//设置焦点->html
		CFindWnd ies(pItem->GetAssocWbw()->m_pExplorer->GetSafeHwnd(), L"Internet Explorer_Server");
		::SetFocus(ies.m_hWnd);
...
void CDlgMainTabResource::OnMainTabChanged()
{
	...
	//设置焦点->html
	CTabItem_WebBrowser *pItem = (CTabItem_WebBrowser*) m_tabWnd.GetTabItem(Pos);
	if (NULL != pItem)
	{
		CFindWnd ies(pItem->GetAssocWbw()->m_pExplorer->GetSafeHwnd(), L"Internet Explorer_Server");
		::SetFocus(ies.m_hWnd);
...
LRESULT CDlgMainTabResource::WindowProc(UINT message, WPARAM wParam, LPARAM lParam)
{
