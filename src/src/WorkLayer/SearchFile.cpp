/* 
 * $Id: SearchFile.cpp 14238 2009-07-08 10:04:38Z dgkang $
 * 
...
CSearchFile::CSearchFile(uint32 nSearchID,LPCTSTR szFileName,EMFileSize nFileSize,int nCompleteCount,int nWeeklyStart,LPCSTR szHash)
{
	m_nSearchID = nSearchID;
	m_bKademlia = m_bServerUDPAnswer = false;

	m_nClientID = m_nClientPort = 0;

	// FT_COMPLETE_SOURCES is not yet supported by all servers, we may have to create such a tag..
	CTag* pTag = new CTag(FT_SOURCES, nWeeklyStart);
	taglist.Add(pTag);
	pTag = new CTag(FT_COMPLETE_SOURCES, nCompleteCount);
	taglist.Add(pTag);

	//AddCompleteSources(nCompleteCount);

	strmd4(szHash,m_abyFileHash);
