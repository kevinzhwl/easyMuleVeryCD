/* 
 * $Id: ED2KLink.cpp 19920 2010-06-22 06:50:01Z dgkang $
 * 
...
CED2KLink* CED2KLink::CreateLinkFromUrl(const TCHAR* uri)
{
	...
	if (strTok == _T("ed2k://"))
	{
		...
		// RSS link
		else if ( strTok == _T("subscribe") )
		{
			CString strURL = GetNextString(strURI, _T("|"), iPos).Trim();
			if ( !strURL.IsEmpty() && GetNextString(strURI, _T("|"), iPos) == _T("/") )
			{
				int iCheckPos = -1;
				...
				if ( iCheckPos != -1 )
				{
					if ( strURL[iCheckPos] == TCHAR('/') )
					{
						strURL.Insert(iCheckPos, TCHAR(':'));
					}
				}
				return new CED2KRssLink(strURL);
			}
		}
