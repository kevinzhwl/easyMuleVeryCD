/* 
 * $Id: PPgDisplay.cpp 19812 2010-06-08 11:34:14Z dgkang $
 * 
...
BEGIN_MESSAGE_MAP(CPPgDisplay, CPropertyPage)
	...
	ON_BN_CLICKED(IDC_CHECK_SHOW_INFO_WINDOW, &CPPgDisplay::OnBnClickedShowInfoWindow)
END_MESSAGE_MAP()
...
void CPPgDisplay::LoadSettings(void)
{
	...
	CheckDlgButton( IDC_CHECK_SHOW_INFO_WINDOW, (UINT)thePrefs.m_bShowInfoWindow );
...
void CPPgDisplay::Localize(void)
{
	if(m_hWnd)
	{
		...
		SetDlgItemText( IDC_CHECK_SHOW_INFO_WINDOW, GetResString(IDS_SHOW_INFO_WINDOW) );
	}
}
...
void CPPgDisplay::OnBnClickedShowInfoWindow()
{
	thePrefs.m_bShowInfoWindow = IsDlgButtonChecked( IDC_CHECK_SHOW_INFO_WINDOW );
	SetModified(TRUE);
}