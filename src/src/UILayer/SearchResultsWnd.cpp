/* 
 * $Id: SearchResultsWnd.cpp 20835 2010-11-18 10:36:09Z dgkang $
 * 
...
CSearchResultsWnd::~CSearchResultsWnd()
{
	...
	//if (m_uTimerUpdataSeachStat)
	//	VERIFY( KillTimer(m_uTimerUpdataSeachStat) );
...
void CSearchResultsWnd::OnInitialUpdate()
{
...
#ifndef EMULE_SEARCH 
	m_ctlMethod.ShowWindow(SW_HIDE);
	CRect Rc,rect;
	m_ctlMethod.GetClientRect(rect);
	m_SearchCtrl.GetWindowRect(&Rc);
	Rc.left -= rect.Width() + 10;
	this->ScreenToClient(&Rc);
	m_SearchCtrl.SetWindowPos(NULL,Rc.left,Rc.top,0,0,SWP_NOZORDER | SWP_NOSIZE);

	m_BtnSearch.GetWindowRect(&Rc);
	ScreenToClient(&Rc);
	Rc.left -= rect.Width() + 10;
	m_BtnSearch.SetWindowPos(NULL,Rc.left,Rc.top,0,0,SWP_NOZORDER | SWP_NOSIZE);

	m_StcSeachInfo.GetWindowRect(&Rc);
	ScreenToClient(&Rc);
	Rc.left -= rect.Width() + 10;
	m_StcSeachInfo.SetWindowPos(NULL,Rc.left,Rc.top,Rc.Width()+200,Rc.Height(),SWP_NOZORDER );
#else
	CRect Rc;
	m_StcSeachInfo.GetWindowRect(&Rc);
	ScreenToClient(&Rc);
	...
	m_StcSeachInfo.SetWindowText(_T(""));
...
#else
	VERIFY( m_ctlMethod.AddItem(GetResString(IDS_SEARCHVERYCD), 0) ==  SearchAreaEasyMuleFile);
#endif
...
void CSearchResultsWnd::DoDataExchange(CDataExchange* pDX)
{
	...
	DDX_Control(pDX, IDC_STATIC_SEARCHRESULT, m_StcSeachInfo);
}
...
void CSearchResultsWnd::OnTimer(UINT nIDEvent)
{
	...
	else if ( nIDEvent == m_uTimerUpdataSeachStat )
	{
		bool bUpdata = theApp.emuledlg->m_mainTabWnd.m_dlgSearch.UpdataJasonSearchState();

		if (!bUpdata)//没有需要刷新的搜索
		{
			VERIFY( KillTimer(m_uTimerUpdataSeachStat) );
			m_uTimerUpdataSeachStat = 0;
		}
...
#else
	iml.Add(CTempIconLoader(IDI_ICON_VERYCD,16 ,16));
#endif
...
void CSearchResultsWnd::Localize()
{
	...
	theApp.emuledlg->m_mainTabWnd.m_dlgSearch.LocalizeJasonSearchState();
...
bool CSearchResultsWnd::StartNeweasyMuleFileSearch(SSearchParams* pParams)
{
	...
	m_uTimerUpdataSeachStat = 32770;
	SetTimer(m_uTimerUpdataSeachStat, 750, NULL);
...
bool CSearchResultsWnd::DoNewKadSearch(SSearchParams* pParams,UINT uSearchID/*=0*/)
{
	...
	// robert, 2010-05-26, 非法字符自动去除 begin
	if (pParams->strKeyword.IsEmpty()){
		CString strError;
		strError.Format(GetResString(IDS_KAD_SEARCH_KEYWORD_INVALID), _aszInvKadKeywordChars);
		throw new CMsgBoxException(strError, MB_ICONWARNING | MB_HELP, eMule_FAQ_Search - HID_BASE_PROMPT);
	} else if(pParams->strKeyword.FindOneOf(_aszInvKadKeywordChars) != -1) {
		while(true) {
			int i = pParams->strKeyword.FindOneOf(_aszInvKadKeywordChars);
			if(i == -1)
				break;
			pParams->strKeyword.Delete(i);
		}
	}
	// robert, 2010-05-26, 非法字符自动去除 end
	...
	try
	{
		...
		if (!pSearch){
			// robert, 2010-05-26, PrepareFindKeywords()可以返回NULL
			//ASSERT(0);
			return false;
		}
	}
...
void CSearchResultsWnd::OnSysCommand(UINT nID, LPARAM lParam)
{
	if (nID == SC_KEYMENU)
	{
		//modify by zhuhui on 2010-5-25 : 现在不需要这个快捷菜单了
...
void CSearchResultsWnd::UpdateParamDisplay(SSearchParams* pParams)
{
	...
	theApp.emuledlg->m_mainTabWnd.m_dlgSearch.UpdataActiveJasonSearchState(pParams);
