/* 
 * $Id: otherfunctions.cpp 20251 2010-07-22 06:49:14Z dgkang $
 * 
...
CString URLEncode(const CString& sInT,bool bUtf8,bool bEncodeReserved)
{
	CStringA sIn;

	if(bUtf8)
		sIn = CStringA(StrToUtf8(sInT));
	else
		sIn = CStringA(sInT);
	...
	while (*pInBuf)
	{
		...
		else if( !bEncodeReserved && _mbschr((unsigned char *)":/?#[]@;=", (BYTE)*pInBuf) )
		{
			pInBuf++;
			continue;
		}
		else
		{
			if( (BYTE)*pInBuf=='%' )
			{
				if( *(pInBuf+1) && *(pInBuf+2) && IsHexDigit(*(pInBuf+1)) && IsHexDigit(*(pInBuf+2)) )
				{					
					pInBuf++;
					continue;
				}				
...
CString CreateED2kLink(const CAbstractFile* pFile, bool bEscapeLink,bool bKeepSpaces)
{
	...
	strLink.Format(_T("ed2k://|file|%s|%I64u|%s|"),
		EncodeUrlUtf8(StripInvalidFilenameChars(strFileName, bKeepSpaces)),
		pFile->GetFileSize(),
...
CString PriorityToString(uint8 uPriority, bool bIsAuto)
{
	...
	case PR_VERYHIGH :
		...
		//Xman PowerRelease
	case PR_POWER:
		strPriority = GetResString(IDS_POWERRELEASE);
		break;
		//Xman end
	default:
		strPriority.Empty();
