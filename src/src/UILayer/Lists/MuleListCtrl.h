/*
 * $Id: MuleListCtrl.h 20251 2010-07-22 06:49:14Z dgkang $
 * 
...
public:
	...
	void LoadSettings()
	{
		// always use last config
		LoadSettings(FALSE);
	}
	void LoadSettings(BOOL bCheckColNum);
	...
	enum ArrowType { arrowDown = IDB_DOWN, arrowUp = IDB_UP,
		...
		arrowExpand = IDB_HEADER_EXPAND, arrowCollapse = IDB_HEADER_COLLAPSEC};//RSS海报列展开\收起状态
	...
	ArrowType GetArrowType() { return m_atSortArrow; }
	// Places a sort arrow in a column

	//设置RSS列表的箭头 by huhg
	void SetRssArrow(int iColumn, bool bExpand) { SetRssArrow(iColumn, bExpand ? arrowUp : arrowDown); }
	void SetRssArrow(int iColumn, ArrowType atType, CString strPoster = _T(""));
	//设置RSS列表的箭头 by huhg
