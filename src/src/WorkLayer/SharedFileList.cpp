/* 
 * $Id: SharedFileList.cpp 20835 2010-11-18 10:36:09Z dgkang $
 * 
...
//	CGlobalVariable::filemgr.RemoveFileItem(pFile);
	...
	if (pFile->GetAICHHashset() && pFile->GetAICHHashset()->HasValidMasterHash())
	{
		AICH_MD4 am;
		memcpy(am.m_abyMD4,pFile->GetFileHash(),16);
		memcpy(am.m_abySHA,pFile->GetAICHHashset()->GetMasterHash().GetRawHash(),CAICHHash::GetHashSize());
		CAICHCleanThread::Add(am);
	}
#endif
	return bResult;
}
...
void CSharedFileList::GetSharedFilelist(CArray<PublishFileSt,PublishFileSt&>& ls)
{
	CCKey Key;
	CKnownFile *pKnownFile;

	POSITION Pos = NULL;
	for(Pos = m_Files_map.GetStartPosition(); Pos != NULL;)
	{
		m_Files_map.GetNextAssoc(Pos,Key,pKnownFile);
	}
}
...
void CSharedFileList::SendListToServer(){
	...
	for(pos=m_Files_map.GetStartPosition(); pos!=0;)
	{
		...
		// --->added by robert, 2010-04-21
		// 没有完整part时不发布到eserver
		if(cur_file->IsKindOf(RUNTIME_CLASS(CPartFile)) && 
			((CPartFile*)cur_file)->GetStatus() == PS_EMPTY)
		{
			continue;
		}
		// <---added by robert, 2010-04-21
		...
		if( Kademlia::CKademlia::GetTotalStoreSrc() < KADEMLIATOTALSTORESRC)
		{
			if(tNow >= m_lastPublishKadSrc)
			{
				...
				if(pCurKnownFile)
				{
					// --->added by robert, 2010-04-21
					// 没有完整part时不发布到kad
					if( !(pCurKnownFile->IsKindOf(RUNTIME_CLASS(CPartFile)) && 
						((CPartFile*)pCurKnownFile)->GetStatus() == PS_EMPTY))
					{
